
// Namespace: 
private enum DebugItemScreen.Op 
{
	// Fields
	public int value__; // 0x0
	public const DebugItemScreen.Op P1 = 0;
	public const DebugItemScreen.Op M1 = 1;
	public const DebugItemScreen.Op Config = 2;
	public const DebugItemScreen.Op Init = 3;
	public const DebugItemScreen.Op Max = 4;
	public const DebugItemScreen.Op New = 5;
	public const DebugItemScreen.Op Break = 6;
	public const DebugItemScreen.Op Repair = 7;
	public const DebugItemScreen.Op RepairMenu = 8;
	public const DebugItemScreen.Op RebuyMenu = 9;
	public const DebugItemScreen.Op Dispose = 10;
}